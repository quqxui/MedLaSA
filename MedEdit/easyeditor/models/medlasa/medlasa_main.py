...
from peft import get_peft_model, AdaLoraConfig,LoraConfig, TaskType, get_peft_model_state_dict, set_peft_model_state_dict
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer

from .medlasa_hparams import MedLaSAHyperParams


def apply_medlasa_to_model(
        model: AutoModelForCausalLM,
        tok: AutoTokenizer,
        requests: List[Dict],
        hparams: MedLaSAHyperParams,
        copy=False,
    ...
    if not keep_original_weight and hasattr(model,'peft_config'):
        ...
        if hparams.lora_ver =='AdaLoRA':
            peft_config = AdaLoraConfig(
                ...
                alpha_pattern=hparams.alpha_pattern,
            )
        elif hparams.lora_ver =='LoRA':
            peft_config = LoraConfig(
                ...
                target_modules=hparams.target_modules,
                alpha_pattern=hparams.alpha_pattern,
                rank_pattern=hparams.rank_pattern
            )
