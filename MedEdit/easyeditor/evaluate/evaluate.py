...
def change_alpha_pattern(args, model,prompt2alpha, hparams, prompt):
    save_path = f'results/{args.dataset}-{args.method}'
    if args.lora_ver:
        save_path += f'-{args.lora_ver}'
    if args.lr:
        save_path += f'-{args.lr}'
    if args.num_steps:
        save_path += f'-{args.num_steps}'
    if args.method == 'MedLaSA':
        if args.alpha_dynamic: save_path += f'-alpha_dynamic'
        if args.rank_dynamic: save_path += f'-rank_dynamic'
        save_path += f'-{args.alpha0}'
        save_path += f'-{args.rank0}'
        save_path += f'-{args.norm}'
...
def compute_med_edit_quality(
    ...
    if rephrase_prompts is not None:
        model = change_alpha_pattern(args, model, prompt2alpha, hparams, rephrase_prompts)
        ret.update(
    ...
    if 'locality' in record.keys() and any(record['locality']):
        for locality_key in record['locality'].keys():
            model = change_alpha_pattern(args, model,prompt2alpha, hparams, record['locality'][locality_key]['prompt'])
            ret['locality'].update(
