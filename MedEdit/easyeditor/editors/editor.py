...
from ..evaluate import compute_edit_quality, compute_icl_edit_quality, compute_med_edit_quality
...
class BaseEditor:
    ...
    def mededit(self,
             ...
             locality_inputs:  Optional[Dict] = None,
             portability_inputs: Optional[Dict] = None,
             prompt2alpha: Optional[Dict] = None,
             args = None,
             keep_original_weight=False,
             verbose=True,
             **kwargs
        ...
        for i, request in enumerate(requests):
            start = time()
            self.hparams.alpha_pattern = prompt2alpha[request["prompt"]]
            edited_model, weights_copy = self.apply_algo(
                self.model,
                self.tok,
                [request],
            ...
            all_metrics[i].update({
                ...
                "requested_rewrite": request,
                "time": exec_time,
                "post": compute_med_edit_quality(args, edited_model, prompt2alpha, self.model_name, self.hparams, self.tok, request, self.hparams.device, test_generation=test_generation),
            })
